...
const MESH_KEYWORD = "Mesh";
  ...
  @Component({ 
    selector: "speed-summary-schedule",
    templateUrl: "speed-summary-schedule.html"
  }) 
  
  export class SpeedSummarySchedule {
    ...
    ngOnInit() {
        ...
        if(this.schema.datasource[0].flextype == "schedule"){
          this.flexType = "schedule";
        }else {
          this.flexType = "dynamic";
        }
    ...
    getBEndContrateBw() {
      if(this.flexType === 'dynamic'){
        return this.schema.datasource[0].model[0].contractedBw;
      }else if(this.flexType === 'schedule') {
        return this.schema.datasource[0].model[1].contractedBw;
      }
    } 
