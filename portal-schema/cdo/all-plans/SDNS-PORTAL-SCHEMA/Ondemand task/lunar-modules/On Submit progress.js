...
evaluate = function (){
    console.log("cdo: dyn - submit PC confirm TRACK Progress =>", model, getDataSource('review'), getDataSource('configure'));
    let _ = getLodash();
    ...
    console.log("cdo: dyn -datasource TRACK Progress", dataSource);
    if(dataSource && dataSource[0] && dataSource[0].model && dataSource[0].model.length>0) {
       ...
       console.log("cdo: dyn - RESOURCE ID", rid);
   ...
   //    });
   (result) => {
       console.log("cdo: Result DBW TRACKSUBMIT =>", result);
      ...
      // result.selectedProduct = product;
       ...
       console.log("cdo dyn result track", result ); 
