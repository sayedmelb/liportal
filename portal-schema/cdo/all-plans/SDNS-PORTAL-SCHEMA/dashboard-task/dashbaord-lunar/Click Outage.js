...
evaluate = function (){
    ...
    if (LPDS[0].location){
        ...
        LPDS[0].location.forEach ((location)=>{
            console.log("filter results =>",filterByStatus(location.properties.products, 'outage'));
            if(filterByStatus(location.properties.products, 'outage')) {
                map.push({
                    address: location,
...
var filterByStatus = function(products, status) {
    ...
    _.forEach(products, product => {
        if(product.properties.status) {
            ...
            if(servStatus && servStatus.type && servStatus.type == 'outage') {
                count ++;
            }
        }
    });
